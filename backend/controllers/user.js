...
exports.signup = (req, res, next) => {
  // logique de la route signup avant exportation dans le routeur
  bcrypt
    .hash(req.body.password, 10) // utilisation du plugin bcrypt (créer un hash du password (utilisation de l'algo 15 fois = salage du mot de passe))
    .then((hash) => {
      delete req.body.passwordCheck; // recupération du hash pour :
      const user = new User({
        // nouvelle instance du schema user
...
exports.login = (req, res, next) => {
  User.findOne({ email: req.body.email }) //User findOne permet de ne séléctionner qu'un élément du schema présent dans mongo atlas et req body email séléctionne les données  du champ saisie
    .then((user) => {
      // Si 'user' appeler et retrouver dans la base de donnée
      if (user === null) {
        res
          .status(401)
          .json({ message: "Paire identifiant/mot de passe incorrecte" });
      } else {
        // sinon(l'utilisateur existe dans la base de donnée donc:)
        bcrypt
          .compare(req.body.password, user.password) // Comme le mot de passe n'est pas enregistré (uniquement le hash et que l'on utilise un algorytme non réversible )
          .then((valid) => {
            if (!valid) {
              // Si la réponse n'est pas valid (les deux hashs ne correspondent pas)
              res
                .status(401)
                .json({ message: "Paire identifiant/mot de passe incorrecte" });
            } else {
              // Si les hash correspondent
              res.status(200).json({
                // Si valid :
                userId: user._id, // renvoie le user id généré par mongoDB + un token
                token: jwt.sign(
                  //  création du TOKEN
                  { userId: user._id, isAdmin: user.isAdmin }, // premier argument séléction de la chose a encodé ici l'id généré par mongo DB cela permet de fixer des droits par rapport a un utilisateur
